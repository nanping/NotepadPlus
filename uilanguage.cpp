...
UILanguage *UILanguage::pLanObj=new UILanguage();

UILanguage::UILanguage():XmlFile (),
    lanTypes({
{"English", "english.xml"},
...
void UILanguage::setLanguagePath(const QString &path)
{
    lanPath.clear();
    if(path.isEmpty())
    {
...
bool UILanguage::load(const QString &lanType)
{
...
bool UILanguage::getMainMenus(QMap<QString, QString> &menus)
{
    return getNodesSameAttr("/NotepadPlus/Native-Langue/Menu/Main/Entries/Item","menuId","name",menus,true,true);
}

bool UILanguage::getSubMenus(QMap<QString, QString> &menus)
{
    return getNodesSameAttr("/NotepadPlus/Native-Langue/Menu/Main/SubEntries/Item","subMenuId","name",menus,true,true);
}

bool UILanguage::getCommands(QMap<QString, QString> &commands)
{
    return getNodesSameAttr("/NotepadPlus/Native-Langue/Menu/Main/Commands/Item","id","name",commands,true,true);
}

bool UILanguage::getTabMenus(QMap<QString, QString> &menus)
{
    return getNodesSameAttr("/NotepadPlus/Native-Langue/Menu/Main/TabBar/Item","CMID","name",menus,true,true);
}

QString UILanguage::getDialogAttr(const QString &objName, const QString &attName, const QString &defValue)
{
    QString x=QString("/NotepadPlus/Native-Langue/Dialog/%1").arg(objName);
    return getAttribute(x,attName,defValue);
}

const QMap<QString, QString> UILanguage::getLanTypes() const
{
    return lanTypes;
}

UILanguage *UILanguage::getInstancePtr()
{
    return pLanObj;
}
